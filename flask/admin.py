...
@admin_bp.route('/caricaEventiEA', methods=['POST'])
def carica_eventi_ea():
  if 'admin' not in request.cookies:
    return jsonify({'status': 'error', 'message': 'Accesso non autorizzato'}), 401
  
  try:
    # Calcola la data di domani nel formato richiesto (gg-mm-yyyy)
    tomorrow = datetime.now() + timedelta(days=1)
    tomorrow_str = tomorrow.strftime('%d-%m-%Y')
    
    # Effettua la richiesta all'API di EasyAcademy
    url = f'https://easyacademy.unipg.it/agendaweb/rooms_call.php?sede=P02E04&date={tomorrow_str}'
    ...
    if not data or 'table' not in data:
      return jsonify({
        'status': 'error',
        'message': 'Formato dati non valido o eventi non trovati nella risposta'
      }), 500
    ...
    for aula_code, aula_data in data['table'].items():
      for slot in aula_data:
        # Controlla se c'è un evento in questo slot (from e to devono essere presenti)
        if isinstance(slot, dict) and 'from' in slot and 'to' in slot and 'NomeAula' in slot:
          # Converte gli orari da stringa a oggetti time
          ora_inizio = datetime.strptime(slot['from'], '%H:%M:%S').time()
          ora_fine = datetime.strptime(slot['to'], '%H:%M:%S').time()
          nome_aula = slot['NomeAula']
          
          # Aggiungi l'evento alla lista per l'inserimento batch
          eventi_data.append((nome_aula, tomorrow.date(), ora_inizio, ora_fine))
          eventi_count += 1
    
    # Inserisci gli eventi nel database
    cursor.executemany("""
      INSERT INTO eventi (aula, data_evento, ora_inizio, ora_fine)
      VALUES (%s, %s, %s, %s)
    """, eventi_data)
    ...
    return jsonify({
      'status': 'success',
      'message': f'Caricamento eventi completato con successo.',
      'details': f"""
        Eventi elaborati:
        - {eventi_count} eventi caricati per la data {tomorrow.strftime('%d/%m/%Y')}
      """
    })
    ...
    return jsonify({
      'status': 'error',
      'message': f"Errore durante l'importazione degli eventi: {str(e)}"
    }), 500
  finally:
    if 'cursor' in locals() and cursor:
      cursor.close()
